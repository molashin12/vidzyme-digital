...
const PromptGenerationOutput = z.object({
  image_prompt: z.string().describe('Generated image prompt for the video')
});
...
async function generatePrompts(input) {
  const { imageAnalysis, userPrompt } = PromptGenerationInput.parse(input);
...
Think Tool: Double-check output for completeness, text accuracy, adherence to UGC realism, and that **only image outputs** are returned.`;
    ...
    const userMessage = `Your task: Create 1 image prompt as guided by your system guidelines.
...
Use the Think tool to double check your output`;
    ...
    try {
      ...
      return {
        image_prompt: parsed.image_prompt || `UGC-style scene: ${userPrompt} featuring ${imageAnalysis.objects.join(' and ')}`
      };
    } catch (parseError) {
      console.warn('Failed to parse JSON response, creating fallback image prompt');

      return {
        image_prompt: `UGC-style scene: ${userPrompt} featuring ${imageAnalysis.objects.join(' and ')}`
      };
    }
  } catch (error) {
...
Use the Think tool to double check your output`;
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        if (!parsed.image_prompt || !parsed.aspect_ratio_image) {
          throw new Error('Invalid response format');
        }

        return {
          image_prompt: parsed.image_prompt,
          aspect_ratio_image: parsed.aspect_ratio_image
        };
      } else {
